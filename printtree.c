...
static void pr_frag(FILE *out, F_frag frag)
{
  switch(frag->kind){
    case F_stringFrag:
      ...
      break;
    case F_procFrag:
      fprintf(out, "%s:\n", Temp_labelstring(F_name(frag->u.proc.frame)));
      pr_stm(out,frag->u.proc.body, 0);
  }
}

void printFragList (FILE *out, F_fragList fragList) 
{
  for (; fragList; fragList=fragList->tail) {
    pr_frag(out, fragList->head); fprintf(out, "\n");
  }
}